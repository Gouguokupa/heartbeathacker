...
from models.model_1 import IEGMNet, IEGMNetSimple5a, IEGMNet1, IEGMNet2, MyNet4, IEGMNet3, IEGMNet4, IEGMNet5, IEGMNet6, IEGMNet2_quant
...
def main():
    ...
    best_model_path = './saved_models/best_IEGM_net.pkl'
    for epoch in range(epoch_num):  # loop over the dataset multiple times (specify the #epoch)
        ...
        for j, data in enumerate(trainloader, 0):
            inputs, labels = data['IEGM_seg'].squeeze(3), data['label']
        ...
        for data_test in testloader:
            net.eval()
            IEGM_test, labels_test = data_test['IEGM_seg'].squeeze(3), data_test['label']
        ...
        if test_accuracy > best_test_accuracy:
            ...
            print('Saved the model with the best testing accuracy: %.5f' % best_test_accuracy)
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--lr', type=float, help='learning rate', default=0.001) #1e-2
    argparser.add_argument('--batchsz', type=int, help='total batchsz for traindb', default=32) #128
    argparser.add_argument('--cuda', type=int, default=None)
    argparser.add_argument('--size', type=int, default=1250)
    argparser.add_argument('--path_data', type=str, default='C:\\Users\\kotha\\Documents\\tinymlcontest2023_demo_example\\tinyml_contest_data_training\\')
